...
ui <- fluidPage(
  ...
  fluidRow(
    column(width = 7, ggiraphOutput("DeMMO_sites", click="plot_click")),
...
server <- function(input, output) {
  ...
  #})
    ...
    DeMMO_map <- ggplot(site_coords, aes(x, y, tooltip = Site, asp=1)) +
      annotation_custom(rasterGrob(imgage, 
                                   width = unit(1,"npc"), 
                                   height = unit(1,"npc"))) +
      geom_point_interactive(size=5, color="#AFD36C") +
    ...
    x <- girafe(ggobj = DeMMO_map )
    output$DeMMO_sites <- renderggiraph({
      ggiraph(code = print(DeMMO_map) )
    })
